...
def discriminator(inp, is_training, init=False):
    ...
    with tf.name_scope('D_model'):
        ...
        dis_logit = nn.dense(x, 10, nonlinearity=None, init=init, counters=counter, train_scale=True, init_w=init_w)
        dis_prob = tf.sigmoid(dis_logit)

    with tf.name_scope('Q_model'):
        x = nn.dense(x, 250, nonlinearity=tf.nn.relu, init=init, counters=counter, train_scale=False, init_w=init_w)
        x = nn.dense(x, 250, nonlinearity=tf.nn.relu, init=init, counters=counter, train_scale=False, init_w=init_w)
        q_logit = nn.dense(x, 10, nonlinearity=None, init=init, counters=counter, train_scale=True, init_w=init_w)
        q_prob = tf.nn.softmax(q_logit)

    return dis_logit, q_logit

def generator(batch_size,c,is_training):

    z_seed = tf.random_uniform([batch_size, 100],name='z_seed')
    x = tf.concat(z_seed, c)
