...
def generator(z_seed, code, is_training, init=False,reuse=False):
    with tf.variable_scope('generator_model', reuse=reuse):
        counter = {}
        onehot_code =tf.cast(tf.one_hot(code,depth=10),dtype=tf.float32)
        x = tf.concat([z_seed, onehot_code],axis=1)
