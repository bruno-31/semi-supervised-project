...
from bruno_gan import generator, discriminator
...
def main(_):
    ...
    with tf.name_scope('optimizers'):
        ...
        cvars = [var for var in tvars if 'discriminator_model/Q_weights' in var.name]
        ...
        maintain_averages_op = ema.apply(dvars+cvars)
        with tf.control_dependencies([train_cls_op]):
            train_cls_op_ema = tf.group(maintain_averages_op)

        copy_graph = [tf.assign(x, ema.average(y)) for x, y in zip(testvars, dvars+cvars)]
    ...
    with tf.name_scope('summary'):
        ...
        with tf.name_scope('cls_summary'):
            tf.summary.scalar('loss_cls', loss_q, ['cls'])
        ...
        for epoch in range(200):
            ...
            for t in range(nr_batches_train):
                ...
                feed_dict = {inp: trainx[ran_from:ran_to],
                             ...
                             is_training_pl: True,
                             lbl_fake: cat_z()}
                _, lc, acc, sm = sess.run([train_cls_op_ema, loss_q, accuracy_cls, sum_op_cls], feed_dict=feed_dict)
                train_acc += acc
                train_loss_cls += lc
                writer.add_summary(sm, batch)
