...
flags.DEFINE_float('condfm', 0.1, 'unlabeled weight [1.]')
flags.DEFINE_float('var', 0.1, 'unlabeled weight [1.]')
...
def main(_):
    ...
    gen = mnist_gan_code.generator
    dis = mnist_gan_code.discriminator
    code = tf.random_uniform([FLAGS.batch_size],minval=0,maxval=10,dtype=tf.int32)
    z_seed = tf.random_uniform([FLAGS.batch_size, 100], name='z_seed')
    with tf.variable_scope('generator_model'):
        gen_inp = gen(batch_size=FLAGS.batch_size, is_training=is_training_pl, code=code, z_seed=z_seed)

    with tf.variable_scope('discriminator_model') as scope:
        init_weight_op, _ = dis(inp, is_training_pl, True)
        scope.reuse_variables()
        logits_lab, layer_lbl = dis(inp, is_training_pl)
    ...
    with tf.name_scope('loss_functions'):
        ...
        # dpart2 = tf.dynamic_partition(layer_lbl, partitions=lbl, num_partitions=10)
        #
        # l = []
        # for i in range(10):
        ...
        k=[]
        for j in range(10):
            grad = tf.gradients(logits_gen[j], z_seed)
            k.append(grad)
        J=tf.stack(k)
        J = tf.squeeze(J)
        ...
        # k=[]
        # for j in range(10):
        #     grad = tf.gradients(logits_unl[j], unl)
        #     val_nan =
        #     m1 = tf.where(tf.is_nan(val_mean), val_nan, grad)
        #     k.append(m1)
        # J=tf.stack(k)
        # J = tf.squeeze(J)
        # J = tf.transpose(J,perm=[1,0,2]) # jacobian
        # j_n = tf.square(tf.norm(J,axis=[1,2]))
        # j_loss_unl = tf.reduce_mean(j_n)
        #
        # if FLAGS.nabla:
        #     loss_dis += 0.001 * j_loss_gen + 0.001 * j_loss_unl
        #     loss_gen += 0.001 * j_loss_gen + 0.001 * j_loss_unl

        #
        # grad = tf.gradients(logits_gen, z_seed)
    ...
    with tf.name_scope('summary'):
        with tf.name_scope('dis_summary'):
            ...
            # tf.summary.scalar('loss_var', loss_var,['dis'])
            # tf.summary.scalar('loss_condfm',loss_condfm,['dis'])
            tf.summary.scalar('loss_jnorm_gen',j_loss_gen,['dis'])
            tf.summary.scalar('loss_jnorm_unl',j_loss_unl,['dis'])
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(200):
            ...
            for t in range(nr_batches_train):
                ...
                # train generator
                _, lg, sm = sess.run([train_gen_op, loss_gen, sum_op_gen], feed_dict={inp: trainx[ran_from:ran_to],
                                                                                      lbl: trainy[ran_from:ran_to],
