...
def main(_):
    ...
    gen = cifar_gan_code.generator
    dis = cifar_gan_code.discriminator
    ...
    gen(random_z, code,is_training_pl, init=True)
    gen_inp = gen(random_z, code,is_training_pl, init=False,reuse=True)

    dis(unl, is_training_pl, init=True)
    logits_lab, layer_lbl = dis(inp, is_training_pl, init=False,reuse=True)
    ...
    with tf.name_scope('loss_functions'):
        ...
        for i in range(10):
            ...
            m22 = tf.where(tf.is_nan(tf.reduce_mean(m2)), val_nan, val_mean) # to avoid nan in sum
            l.append(m22)
        loss_condfm = tf.reduce_mean(l)

        loss_gen = loss_fm + loss_condfm
    ...
    with tf.name_scope('summary'):
        ...
        with tf.name_scope('gen_summary'):
            ...
            tf.summary.scalar('loss_condFM', loss_condfm,['gen'])
