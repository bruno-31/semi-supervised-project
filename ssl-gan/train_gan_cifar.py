...
def main(_):
    ...
    with tf.variable_scope('generator_model', reuse=True):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(200):
            ...
            for t in range(nr_batches_train):
                ...
                _, ll, lu, acc = sess.run([train_dis_op, loss_lab, loss_unl, accuracy],
                                              feed_dict=feed_dict)
                train_loss_lab += ll
                train_loss_unl += lu
                train_acc += acc
                ...
                # train generator
                _, lg = sess.run([train_gen_op, loss_gen], feed_dict={unl: trainx_unl2[ran_from:ran_to],
                                                                                      is_training_pl: True})
                train_loss_gen += lg
                train_batch += 1
